...
pub trait Repository: Send + Sync {
  fn insert_one(&self, number: PokemonNumber, name: PokemonName, types: PokemonTypes) -> Result<Pokemon, InsertError>;

  fn fetch_all(&self) -> Result<Vec<Pokemon>, FetchAllError>;

  fn fetch_by_name(&self, name: String) -> Result<Vec<Pokemon>, FetchManyError>;

  fn fetch_one_by_number(&self, number: PokemonNumber) -> Result<Pokemon, FetchOneError>;

  fn delete_one_by_number(&self, number: PokemonNumber) -> Result<(), DeleteError>;
}
...
impl Repository for InMemoryRepository {
  ...
  fn fetch_by_name(&self, name: String) -> Result<Vec<Pokemon>, FetchManyError> {
    if self.error {
      return Err(FetchManyError::Unknown)
    }

    let lock = match self.pokemons.lock() {
      Ok(lock) => lock,
      _ => return Err(FetchManyError::Unknown),
    };

    let mut pokemons = lock.to_vec();
    pokemons = pokemons.into_iter().filter(
      |p| String::from(p.name.clone()).to_lowercase().contains(&*name.to_lowercase())
    ).collect::<Vec<Pokemon>>();
    Ok(pokemons)
  }
